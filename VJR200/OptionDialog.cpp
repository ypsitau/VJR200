...
BOOL CALLBACK Tab1Proc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_HSCROLL:
		...
		{
			pos3 = SendMessage(hBar3, TBM_GETPOS, NULL, NULL);
		}
		wsprintf(str, _T("%d ms"), pos3);
		SetWindowText(GetDlgItem(hDlg, IDC_OPTION_QUICKTYPEVIEW), (LPCTSTR)str);
		return (INT_PTR)TRUE;
	...
	case WM_COMMAND:
		...
		{
			HWND hCmbo = GetDlgItem(hDlg, IDC_OPTOIN_LANGUAGE);
			g_language = SendMessage(hCmbo, CB_GETCURSEL, 0, (LPARAM)0);
			...
			g_quickTypeS = SendMessage(hBar3, TBM_GETPOS, NULL, NULL);
...
BOOL CALLBACK Tab2Proc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_HSCROLL:
		...
		{
			pos1 = SendMessage(hBar1, TBM_GETPOS, NULL, NULL);
		}
		wsprintf(str, _T("%d %%"), pos1);
		SetWindowText(GetDlgItem(hDlg, IDC_OPTION_CPUCLOCKVIEW), (LPCTSTR)str);
		return (INT_PTR)TRUE;
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			g_cpuScale = SendMessage(hBar1, TBM_GETPOS, NULL, NULL);
			...
			g_ramInitPattern = SendMessage(hInitList, LB_GETCURSEL, NULL, NULL);
			EndDialog(hDlg, LOWORD(wParam));
			return (INT_PTR)TRUE;
			break;
		}
...
BOOL CALLBACK Tab3Proc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_HSCROLL:
		...
		{
			pos2 = SendMessage(hBar2, TBM_GETPOS, NULL, NULL);
			wsprintf(str, _T("%d %%"), pos2);
			SetWindowText(GetDlgItem(hDlg, IDC_OPTION_VOLUMEVIEW), (LPCTSTR)str);
		}
		...
		{
			stereo1 = SendMessage(hStereo1, TBM_GETPOS, NULL, NULL);
		}
		if (GetDlgItem(hDlg, IDC_OPTION_STEREO2) == (HWND)lParam)
		{
			stereo2 = SendMessage(hStereo2, TBM_GETPOS, NULL, NULL);
		}
		if (GetDlgItem(hDlg, IDC_OPTION_STEREO3) == (HWND)lParam)
		{
			stereo3 = SendMessage(hStereo3, TBM_GETPOS, NULL, NULL);
		}
		if (GetDlgItem(hDlg, IDC_OPTION_WRITEINTERVAL) == (HWND)lParam)
		{
			sBufferWriteInterval = SendMessage(hWriteInterval, TBM_GETPOS, NULL, NULL);
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			g_soundVolume = SendMessage(hBar2, TBM_GETPOS, NULL, NULL);
			sys.pMn1271->SetVolume(g_soundVolume);

			g_stereo1 = SendMessage(hStereo1, TBM_GETPOS, NULL, NULL);
			g_stereo2 = SendMessage(hStereo2, TBM_GETPOS, NULL, NULL);
			g_stereo3 = SendMessage(hStereo3, TBM_GETPOS, NULL, NULL);
			g_sBufferWriteInterval = 100 - 10 * SendMessage(hWriteInterval, TBM_GETPOS, NULL, NULL);
...
BOOL CALLBACK Tab5Proc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			TCHAR str[10];
			GetWindowText(GetDlgItem(hDlg, IDC_OPTION_JOYPAD1ACODE), str, sizeof(str));
			g_Joypad1pA = _ttoi(str);
			GetWindowText(GetDlgItem(hDlg, IDC_OPTION_JOYPAD1BCODE), str, sizeof(str));
			g_Joypad1pB = _ttoi(str);
			GetWindowText(GetDlgItem(hDlg, IDC_OPTION_JOYPAD2ACODE), str, sizeof(str));
			g_Joypad2pA = _ttoi(str);
			GetWindowText(GetDlgItem(hDlg, IDC_OPTION_JOYPAD2BCODE), str, sizeof(str));
			g_Joypad2pB = _ttoi(str);

			GetWindowText(GetDlgItem(hDlg, IDC_OPTION_FORCEDACODE), str, sizeof(str));
			g_forcedJoypadA = _ttoi(str);
			GetWindowText(GetDlgItem(hDlg, IDC_OPTION_FORCEDBCODE), str, sizeof(str));
