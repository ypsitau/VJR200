...
int g_debug = -1; // -1 = 通常実行　　1 = ステップ実行　　0 = 停止
int g_breakPoint[BREAKPOINT_NUM] = { -1 };
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			if (msg.message == WM_QUIT) {
				...
				if (!TranslateAccelerator(g_hMainWnd, hAccelTable, &msg)) {
					if (hWndDebug == 0 || !IsDialogMessage(hWndDebug, &msg)) {
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		{
			int idx = (wParam & 0xFFFF) - IDM_FILE_STATESAVE0;
		...
		{
			int idx = (wParam & 0xFFFF) - IDM_FILE_STATELOAD0;
		...
		{
			int idx = (wParam & 0xFFFF) - IDM_FILE_RFILES_SETCJR0;
...
BOOL CALLBACK DlgDebugProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_COMMAND:
		...
		{
			UINT ctlList[BREAKPOINT_NUM] = { IDC_DEBUG_BREAKPOINT, IDC_DEBUG_BREAKPOINT2, IDC_DEBUG_BREAKPOINT3,
		...
		case IDC_DEBUG_SET:
		{
			UINT ctlList[6] = { IDC_DEBUG_BREAKPOINT, IDC_DEBUG_BREAKPOINT2, IDC_DEBUG_BREAKPOINT3,
				IDC_DEBUG_BREAKPOINT4, IDC_DEBUG_BREAKPOINT5, IDC_DEBUG_BREAKPOINT6 };
			TCHAR buff[6];

			for (int i = 0; i < BREAKPOINT_NUM; ++i) {
				GetDlgItemText(hwnd, ctlList[i], buff, 6);
				TCHAR* e;
...
void SetMenuItemForStateSaveLoad()
{
	...
	for (unsigned int i = 0; i < 10; ++i) {
		...
		miiSave.cch = _tcslen(saveText);
		miiLoad.dwTypeData = loadText;
		miiLoad.cch = _tcslen(loadText);
